...
def add_operator(mcc, mnc, brand, operator, status, country, country_code, db):
    ...
    assert status.lower in ['discontinued', 'inactive', 'not operational', 'operational', 'planned', 'planning', 'reserved', 'returned spare', 'temporary operational', 'suspended', 'unknown', '']
    db.append({
        'mccmnc': mcc + mnc,
        'brand': brand,
        'operator': operator,
