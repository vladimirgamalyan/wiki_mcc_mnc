...
def add_operator(mcc, mnc, brand, operator, status, country, country_code, db):
    ...
    assert status in ['discontinued', 'inactive', 'not operational', 'operational', 'planned', 'planning', 'reserved', 'returned spare', 'temporary operational', 'unknown', 'test network', 'allocated', '']
    db.append({
        'mccmnc': mcc + mnc,
        'brand': brand,
        'operator': operator,
